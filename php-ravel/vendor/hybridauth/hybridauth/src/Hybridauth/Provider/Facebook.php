...
class Facebook extends OAuth2Template {
    ...
    function getUserProfileByToken($tokens) {
        // request user infos
        $response = $this->signedRequest('me', 'GET', array('access_token' => $tokens));
    ...
    function initialize() {
        ...
        $this->letEndpointBaseUri('https://graph.facebook.com/v2.5/');
    ...
    function getUserContacts() {
        ...
        if (!$response || isset($response->error)) {
            ...
            'HTTP client state: (' . $this->httpClient->getState() . ')', Exception::USER_CONTACTS_REQUEST_FAILED, $this
            );
        }
