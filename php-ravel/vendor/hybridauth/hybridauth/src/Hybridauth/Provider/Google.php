...
class Google extends OAuth2Template {
    ...
    function initialize() {
        ...
        $this->letEndpointBaseUri('https://www.googleapis.com/oauth2/v1/');
    ...
    function getUserProfile() {
        ...
        if (!$response || !isset($response->id)) {
            ...
            'HTTP client state: (' . $this->httpClient->getState() . ')', Exception::AUTHENTIFICATION_FAILED, $this
            );
        }
    ...
    function getUserProfileByToken($tokens) {
        $response = $this->signedRequest("userinfo",'GET',array('access_token'=>$tokens));
        $response = json_decode($response);
    ...
    function getUserContacts($args = array()) {
        ...
        if (!$response || isset($response->error)) {
            ...
            'HTTP client state: (' . $this->httpClient->getState() . ')', Exception::USER_PROFILE_REQUEST_FAILED, $this
            );
        }
